<div class="relative lg:mx-auto w-full lg:max-w-310 -mt-16 sm:-mt-30 md:-mt-66 overflow-hidden">
    <div
        ...
        <x-ui.button size="sm" intent="neutral">
            <x-ui.icon name="ph--plus" class="mr-2 size-3.5" />
            Create User
        </x-ui.button>
    ...
    <div
        ...
        <div
            ...
            <div class="flex overflow-hidden">
                ...
                <label for="option1"
                    class="px-3.5 h-7 truncate peer-checked:bg-foreground peer-checked:text-background  w-full flex items-center justify-center">
                    Option 1
                </label>
            </div>
            <div class="flex overflow-hidden">
    ...
    <div
        ...
        <div class="px-5 flex-1 flex items-center md:w-xs h-12 sm:h-20">
            <x-ui.range name="range_slide" min="0" max="100" class="text-foreground"
                custom-indicator="bg-foreground" />
        </div>
    </div>
    <div class="w-full max-lg:overflow-hidden px-px pb-px border-t border-border-strong border-dashed sm:border-t-0">
        <div
            ...
            <div class="grid gap-4">
                <div class="relative z-11 pt-3 md:h-12 flex items-end gap-2 bg-linear-to-t from-background from-80%">
                    <x-ui.switch size="sm" checked name="switch_1" id="switch_1" />
                    <x-ui.switch size="sm" name="switch_2" id="switch_2" bg="default" track="title" knob="bg" />
                </div>
                ...
                <x-ui.card class="shadow-sm [--ui-padding:1.125rem]">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="space-y-0.5 flex flex-col">
                            <span class="font-semibold text-sm text-foreground">Tresor Kasenda</span>
                    ...
                    <div class="flex items-center gap-2 text-sm text-muted-foreground mt-2">
                        <x-ui.icon name="ph--buildings" class="flex" />
                        <span>Unoforge</span>
                    </div>
                ...
                <x-ui.card fill="none" class="h-max bg-card [--ui-padding:0.125rem]">
                    <div class="bg-background  inner-radius p-3.5 flex flex-col">
                        ...
                        <div class="flex flex-col pt1">
                            <x-ui.progress max="10" value="3" class="text-foreground" />
                        ...
                        <div class="flex justify-between mt-1.5">
                            <div class="flex -space-x-2">
                                <x-ui.avatar src="/avatar1.webp" alt="Avatar 1" loading="lazy" decoding="async" size="xs"
                                    fetchpriority="low" width="100" height="100" class="border border-muted" />
                            ...
                            <div class="flex gap-2 items-center text-sm text-muted-foreground">
                                <span class="flex items-center text-xs">
                                    <x-ui.icon name="ph--file-plus" class="text-sm mr-0.5" /> 4
                                </span>
                                <span class="flex items-center text-xs">
                                    <x-ui.icon name="ph--chat-centered-text" class="text-sm mr-0.5" /> 4
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div
                class="grid grid-rows-[auto_1fr_auto] border border-border-strong/40 dark:border-border bg-background  dark:bg-card/40 rounded-ui">
                    ...
                    <div class="flex">
                        <x-ui.input.group size="sm">
                            ...
                            <x-ui.input.leading absolute class="text-foreground text-sm [--left-space:8px]">
                                <x-ui.icon name="ph--magnifying-glass" class="flex size-3.5" />
                            </x-ui.input.leading>
                        </x-ui.input.group>
                    </div>
                    <div class="flex items-center gap-2">
                        <x-ui.button variant="outline" size="sm">
                            <x-ui.icon name="ph--export" class="size-3.5 mr-1.5" />
                            Export
                        </x-ui.button>
                        <x-ui.button intent="neutral" size="sm">
                            <x-ui.icon name="ph--plus" class="size-3.5 mr-1.5" />
                            Invite
                        </x-ui.button>
                    </div>
                </div>
                        ...
                        @foreach ($users_demo as $user)
                            <x-ui.table.row>
                                ...
                                <x-ui.table.cell class="pl-2">
                                    <div class="flex items-center gap-2">
                                        @if ($user['info']['avatar'] !== '')
                                            <x-ui.avatar radius="none" class="rounded-ui bg-muted/60" size="sm"
            ...
            <div class="hidden lg:flex flex-col gap-4 pt-0.5 relative">
                <x-ui.card class="-mt-4 relative z-12 shadow-sm flex items-start gap-4">
                    ...
                    <div class="flex-1 flex flex-col">
                        <span class="text-foreground font-semibold">Github</span>
                ...
                <div class="bg-background  rounded-ui ring ring-border-card w-full px-3.5 py-6 flex flex-col items-center text-center">
                    <div class="size-10 rounded-full ui-subtle ui-subtle-gray flex items-center justify-center">
                        <x-ui.icon name="ph--calendar-plus" class="text-xl" /> 
                    </div>
