...
<span aria-hidden="true" class="absolute top-8 h-4 inset-x-0 flex pointer-events-none">
    <div class="xl:max-w-380 flex flex-col justify-between h-4 w-full mx-auto">
        <span class="h-px bg-linear-to-l from-muted/80 to-pattern-foreground"></span>
        <span class="h-px bg-linear-to-r from-muted/80 to-pattern-foreground"></span>
    </div>
</span>
<header class="px-1.5 max-[350px]:px-2.5 sm:px-4 pt-2 w-full z-45 relative">
    <nav
        class="sm:max-w-4xl bg-background  shadow-sm border border-border/60 dark:border-border-strong shadow-gray-50 dark:shadow-transparent sm:mx-auto w-full flex justify-between ui-card [--card-padding:--spacing(4)] [--card-radius:var(--radius-ui)]">
        ...
        <div class="flex items-center gap-1.5 w-max">
            <x-ui.modal.trigger modal-id="search-modal" size="sm" iconOnly variant="none" :class="$buttonSurfaceClass">
                <x-ui.icon name="ph--magnifying-glass" class="text-sm" />
            </x-ui.modal.trigger>
            <x-ui.button href="https://github.com/unoforge/flexiwind"  size="sm"
                iconOnly variant="none" :class="$buttonSurfaceClass">
                <x-ui.icon name="ph--github-logo" />
            </x-ui.button>
            <x-ui.button  size="sm" iconOnly variant="none" radius="none"
                x-on:click="$store.theme.toggle()" aria-label="toggle theme" :class="'relative ' . $buttonSurfaceClass">
            ...
            <x-atoms.ui-link href="/the-club" aria-label="Link to club page"
                class="ml-1.5 btn h-8 text-sm px-3 btn-solid btn-solid-neutral rounded-ui text-background max-[350px]:hidden"
                wire:navigate>
