...
@if ($isInternalLink)
    <x-atoms.ui-link href="{{ $href }}"
        {{ $attributes->merge(['class' => 'w-max underline decoration-dashed whitespace-normal decoration-1 break-words text-primary rounded-lg ease-linear duration-200 hover:text-primary-800 dark:hover:text-primary-300']) }} wire:navigate>
        {{ $slot }}
    </x-atoms.ui-link>
@else
    <x-atoms.ui-link href="{{ $href }}"
