<x-ui.tabs data-indicator-class-name="ui-tabs-indicator flex bg-background  shadow border border-border-strong/40 rounded absolute top-0 left-0"
    class="w-full max-w-md">
    <x-ui.tabs.list unStyled class="flex items-center relative w-full p-0.5 bg-muted rounded-md">
        <x-ui.tabs.trigger unStyled wrapper-class="flex flex-1" id="accountTab" href="#accountTab" aria-controls="accountTab"
            tabindex="0" class="px-4 py-2 rounded flex-1 flex items-center justify-center text-sm text-foreground relative fx-active:text-foreground">
            Account
        </x-ui.tabs.trigger>
        <x-ui.tabs.trigger unStyled wrapper-class="flex flex-1" id="passTab" href="#passTab" aria-controls="passTab"
            tabindex="-1" class="px-4 py-2 rounded flex-1 flex items-center justify-center text-sm text-foreground relative fx-active:text-foreground">
            Password
        </x-ui.tabs.trigger>
    </x-ui.tabs.list>
    <x-ui.tabs.panel id="accountTab"
        class="p-6 md:p-10 divide-y divide-border-strong border border-border-strong/60 rounded-md mt-2 bg-background  flex-col"
        ...
        <div class="space-y-1.5 pb-6 md:pb-8">
            <h3 class="text-2xl font-semibold leading-none tracking-tight text-foreground">
                Account</h3>
            <p class="text-sm text-foreground">Make changes to your account here.
                Click save
                when you're done.</p>
        </div>
    ...
    <x-ui.tabs.panel id="passTab"
        ...
        <div class="space-y-1.5 pb-6 md:pb-8">
            <h3 class="text-2xl font-semibold leading-none tracking-tight text-foreground">
                Password</h3>
            <p class="text-sm text-foreground">Change your password here. After
